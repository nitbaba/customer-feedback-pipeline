...
def run_migrations():
    ...
    dashboard_secret = get_secret(
        "customer-feedback-pipeline-dev-dashboard-credentials"
    )
    ...
    cursor.execute(
        "SELECT 1 from pg_catalog.pg_database WHERE datname = 'feedback_analytics';"
    )
    ...
    if not cursor.fetchone():
        cursor.execute(
            f"CREATE ROLE dashboard_reader WITH LOGIN PASSWORD %s;",
            (dashboard_password,),
        )
    else:
        cursor.execute(
            f"ALTER ROLE dashboard_reader WITH PASSWORD %s;", (dashboard_password,)
        )
    ...
    cursor.execute(
        "GRANT SELECT ON TABLE product_performance_metrics TO dashboard_reader;"
    )
