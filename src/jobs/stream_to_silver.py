...
def main():
    ...
    spark = (SparkSession.builder
             .appName("CustomerFeedbackSilverStreaming")
             .master("local[4]")
    ...
    silver_enriched_df = bronze_stream_df.withColumn(
        "raw_llm_json",
        serialize_llm_triage_udf(col("review_text"))
    )

    #unpack the raw text JSON coluns string into typed struct elements
    silver_final_df = (silver_enriched_df
                       .withColumn("llm_data", from_json(col("raw_llm_json"), llm_output_schema))
                       .select(
                           col("ticket_id"),
                           col("customer_id"),
                           col("product_id"),
    ...
    query = (silver_final_df.writeStream
             .format("parquet")
             .outputMode("append")
